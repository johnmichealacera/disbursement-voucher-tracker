  ...
  try {
    ...
    if (disbursement.createdBy.role !== "GSO") {
      return NextResponse.json({ 
        error: "Budget Office can only review vouchers from GSO department" 
      }, { status: 403 })
    }
    ...
    const bacHasReviewed = disbursement.auditTrails.some(trail => 
      trail.action === "BAC_REVIEW" && trail.user.role === "BAC"
    )

    if (!bacHasReviewed) {
      return NextResponse.json({ 
        error: "Budget Office can only review GSO vouchers after BAC has reviewed them" 
      }, { status: 400 })
    }
