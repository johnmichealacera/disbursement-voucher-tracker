...
import { cn, formatDateTime, formatTimeDifference } from "@/lib/utils"
    ...
    <div className={cn("w-full", className)}>
      ...
      <div className="space-y-3">
        {steps.map((step, index) => {
          // Calculate time difference from previous completed step
          const previousCompletedStep = index > 0 
            ? steps.slice(0, index).reverse().find(s => s.status === "completed" && s.completedAt)
            : null
          
          const timeDifference = previousCompletedStep && step.completedAt && step.status === "completed"
            ? formatTimeDifference(previousCompletedStep.completedAt!, step.completedAt)
            : null
                ...
                <div className="flex items-center mb-2 ml-11">
                  <div className="flex items-center gap-1.5 px-2.5 py-1 bg-amber-50 border border-amber-200 rounded-md">
                    ...
                    <span className="text-xs font-medium text-amber-700">
                      Processing time: {timeDifference}
                    </span>
                  </div>
                </div>
              )}
